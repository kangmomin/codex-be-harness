...
프로젝트 profile(`.codex/be-harness.local.md`)을 생성/갱신한다.
...
- `.codex/be-harness.local.md` 존재 여부
- 프로젝트 언어 자동 탐지
  - `go.mod` 존재 → `go`
...
| 항목 | 상태 |
|------|------|
| profile (.codex/be-harness.local.md) | 존재 / 없음 |
...
구조화된 사용자 입력 기능이 제공되면 사용하고, 없으면 짧은 일반 질문으로 수집한다. 감지 결과에 맞는 옵션을 기본으로 권장한다.
...
`.codex/be-harness.local.md` 를 아래 형식으로 생성한다.
...
- 기본값:
  ...
  ```json
  {
    "conventions": [
      ...
      { "name": "project", "source": "project", "path": "AGENTS.md" }
    ]
  }
  ```

- `AGENTS.md`가 없으면 `project` 엔트리는 제외한다.
...
```bash
mkdir -p .codex/be-harness/skills .codex/be-harness/agents
```

그리고 안내용 README를 해당 디렉토리에 두되, 이미 존재하면 덮어쓰지 않는다:

```markdown
<!-- .codex/be-harness/README.md -->
...
| .codex/be-harness/ 오버라이드 디렉토리 | 생성 / 이미 존재 |
