...
| 플래그 | 효과 |
|--------|------|
| `-md` / `-html` | 출력 포맷. 플래그(`--twin` 포함)나 사용자 지시·문맥에 포맷이 없으면 질문 |
...
| (범위 없음) | 사용자 지시·문맥에서도 미확정인 항목만 질문: ① 범위 종류(파일/디렉토리/PR/commit range) ② 구체적 값 ③ 문서 초점(변경 요약 (review 용) / 아키텍처 설명 / 온보딩 가이드) — `--brief` 시 ③ 생략 |

사용자나 호출자가 범위·포맷·초점을 이미 정했으면 그 값을 사용해 진행한다. 명시된 값을 플래그로 다시 입력하거나 확인하도록 요구하지 않는다.
