...
`$codex-be-harness:e2e-test` 를 실행하고, 실패가 있으면 수정한 뒤 다시 실행한다. 최대 5회 반복하고, 종료 시 실행 전체를 **정직한 자기 점검 HTML 리포트**로 남긴다.

**플레이스홀더 정의** (본문·assets 공통, 값 변경은 여기 한 곳만 수정):

- `{SKILL_DIR}` = 현재 `SKILL.md`가 있는 디렉토리의 절대 경로
- `{REPORT_PROMPT}` = `{SKILL_DIR}/assets/api-test-cases-prompt.md`
...
`mktemp -d`로 `{RUN_DIR}`을 만들고 절대 경로를 고정한다. 루프 동안 수행하는 모든 테스트 케이스의 요청 데이터·기대·실제·판정과 실패→수정 내역을 `{RUN_REPORT}`에 누적한다. 루프 종료 후 이 파일이 Step 4 HTML 렌더링의 입력이 된다. 서브에이전트에는 두 경로를 모두 절대 경로로 전달한다.
...
1. 같은 플러그인에 포함된 `$codex-be-harness:e2e-test`를 실행한다. 현재 플러그인의 실제 등록 이름으로 resolve하고, 다른 설치나 사용자 전역 스킬로 대체하지 않는다.
   ...
   - 하위 스킬이 `SKIPPED:*`를 반환하면 루프를 추가 진행하지 않는다. `{RUN_DIR}`을 정리하고 동일 SKIP 사유로 보고한다. **Step 4(HTML 렌더링)는 건너뛴다.**
...
2. 결과를 확인한다:
   ...
   - **판정 `FAIL`** → 3번으로 진행
3. 발견된 이슈를 Codex 수정 서브에이전트 한 명에게 위임한다:
   ```
...
**원칙 — `$codex-be-harness:doc-gen` 호출 금지**: doc-gen은 PR/refactor designer 프롬프트가 강제되어 본 보고서의 의도(정직한 자기 점검, GAP 명시, verdict 다운그레이드)와 충돌한다. doc-gen을 거치지 않고 아래 자산 프롬프트를 직접 적용해 단일 standalone HTML을 작성한다.
...
4. 자산 프롬프트 `{REPORT_PROMPT}`를 읽고 본 Step의 렌더링 지시로 사용한다. `{SKILL_DIR}`는 현재 스킬 위치에서 해석하며 현재 작업 디렉토리나 환경 변수로 추측하지 않는다.
...
7. 3에서 결정한 출력 경로에 HTML을 저장한다.
8. 생성된 HTML 절대 경로를 "종료 시 출력"의 `E2E 리포트 HTML:` 줄에 기록한다. 렌더링이 실패하면 `미생성 ({사유})`로 적는다.
9. HTML 저장과 종료 출력용 경로 보관이 끝나면 `{RUN_DIR}`을 정리한다. 정리 실패는 원래 판정을 덮어쓰지 않고 cleanup 경고로 남긴다.
...
| 파일 | 로드 시점 |
|------|----------|
| `{REPORT_PROMPT}` (`assets/api-test-cases-prompt.md`) | Step 4 (HTML 렌더링) |
