...
> **Project Overrides**: 실행 전 `.codex/be-harness/common.md`와 `.codex/be-harness/skills/commit-push.md`를 Read.
...
> **Internal composition**: 다른 workflow 안에서 실행될 때 중첩 skill 호출 기능을 가정하지 않는다. 커밋 단계에서는 [commit](../commit/SKILL.md)을 읽고 해당 절차를 현재 실행 안에서 수행한다.
...
| 그 외 (`codex/*`, `worktree-*`, 임의 이름 등) | 컨벤션 불일치 | Step 1.3 (브랜치 이름 재생성) |
...
프로젝트 오버라이드 **`.codex/be-harness/common.md`** 에 브랜치 모델 표(선언 형식은 플러그인 루트 `OVERRIDES.md` 참조)가 선언돼 있으면:
...
3. `{prefix} → {허용 base}` 매핑은 `$codex-be-harness:commit-pr`의 base 결정과 PR 조합 검증에 사용된다.
...
- `{base}`는 이미 알려진 값이 있으면 재사용한다 — 기존 open PR의 `baseRefName`, `$codex-be-harness:commit-pr` Step 2에서 결정한 base. 없으면 `@{upstream}`, 그것도 없으면 기본 브랜치(`origin/HEAD`)와의 merge-base.
...
- **커밋 메시지** (미push 한정): 마지막 커밋이면 `git commit --amend`, 그 이전 커밋이면 `git reset --soft {범위 시작}` 후 `$codex-be-harness:commit` 절차로 재커밋한다.
- 제거/수정 변경은 관련 논리 단위 커밋에 amend하거나 `Chore: 확인된 Assumption 태그 정리`로 커밋한다.
- 승인된 항목은 **확정된 결정**으로 기록을 이관한다 — PR 흐름(`$codex-be-harness:commit-pr`)이면 PR 본문의 "확정된 결정" 섹션, 아니면 최종 보고에 포함한다.
