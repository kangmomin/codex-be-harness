...
def schema(domain):
    ...
    result.update(language=ENUMS["language"], topologyModels="block")
    if domain == "be":
...
def validate_slot(slot, value):
    if slot not in SLOTS or not isinstance(value, dict) or set(value) - {"model", "effort"}:
        raise ProfileError("INVALID_SLOT: " + slot)
    for key, pattern in (("model", r"[A-Za-z0-9._-]+"),):
        v = value.get(key)
...
class Document:
    ...
    def value(self, key, domain):
        ...
        if kind == "block":
            if raw == "{}" and not children:
                return {}
            if raw:
                raise ProfileError("UNSUPPORTED_LAYOUT: topologyModels must be a block")
            pairs = []
            for j in children:
    ...
    def edit(self, changes, domain):
        ...
        for key, value in changes.items():
            ...
            if kind == "block":
                # An empty patch resets all slots; validate only after repairs.
                merged = dict(old or {}) if value else {}
                for slot, record in value.items():
                    if record is None:
            ...
            if kind == "block" or kind == "array" and children:
                ...
                for child_key, child_value in items:
                    ...
                    if previous == child_value and child_parts[1]:
                        ...
                        continue
                    child_raw = "{ " + ", ".join(k + ": " + child_value[k] for k in ("model", "effort") if k in child_value) + " }" if kind == "block" else encode(child_value, child_parts[1])
                    rendered.append(render_lex(child_parts, child_raw))
                if any("#" in p[2] for p in old_children.values()):
                    raise ProfileError("UNSUPPORTED_LAYOUT: deleting a commented line: " + key)
                if value:
                    rendered.insert(0, render_lex(parts, "") if kind == "block" and parts[1] == "{}" else "".join(parts) if block else key + ":" + eol)
                elif kind == "array":
        ...
        for key, value in expected.items():
            if parsed.value(key, domain) != value and not (key == "topologyModels" and not value and key not in parsed.roots):
                raise ProfileError("INVALID_RENDER: " + key)
        return result


def profile_path(cwd, domain):
    """Resolve the project profile, inheriting only the linked main worktree."""
    cwd = Path(cwd).resolve()
    root = subprocess.run(["git", "-C", str(cwd), "rev-parse", "--show-toplevel"], capture_output=True, text=True)
    project = Path(root.stdout.strip()) if root.returncode == 0 else cwd
    candidate = project / ".codex" / (domain + "-harness.local.md")
    if candidate.exists() or candidate.is_symlink():
        return candidate, False
    if root.returncode == 0:
        common = subprocess.run(["git", "-C", str(project), "rev-parse", "--path-format=absolute", "--git-common-dir"], capture_output=True, text=True)
        if common.returncode:
            raise ProfileError("GIT_CHECK: cannot resolve common directory")
        main = Path(common.stdout.strip()).parent
        inherited = main / ".codex" / (domain + "-harness.local.md")
        if Path(common.stdout.strip()).name == ".git" and main != project and (inherited.exists() or inherited.is_symlink()):
            return inherited, True
    raise ProfileError("NO_PROFILE: " + str(candidate))


def resolve(cwd, domain):
    cwd = Path(cwd)
    profile, inherited = profile_path(cwd, domain)
    ...
    return dict(values=values, sources=sources, commands=commands, diagnostics=diagnostics,
                profile_source=source, profile_path=str(profile), inherited=inherited, writable=source == "profile")
...
def main():
    ...
    try:
        if args.action == "schema":
            ...
            path, inherited = profile_path(args.cwd, args.domain)
            if path.is_symlink():
            ...
            result = dict(status="DONE" if args.apply else "PREVIEW", changed=changed, sha256_before=digest,
                          sha256_after=hashlib.sha256(rendered).hexdigest(), profile_path=str(path), inherited=inherited)
            if not args.apply:
