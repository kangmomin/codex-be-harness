...
def diagnose(cwd, domain, host, tools=None):
    ...
    row("codex", "NATIVE", "native host; no external CLI delegation or provider probe")
    for diagnostic in resolved["diagnostics"]:
        row("topologyModels", "INVALID_SLOT" if "INVALID_SLOT" in diagnostic else "WARN", diagnostic, False)
    failures = {"MISSING", "INVALID", "OUTDATED", "UNKNOWN"}
    status = "ISSUES_FOUND" if any(c["required"] and c["status"] in failures for c in checks) else "CHECKS_INCOMPLETE" if any(c["status"] in {"UNKNOWN", "UNVERIFIED", "WARN", "INVALID_SLOT"} for c in checks) else "AVAILABLE"
