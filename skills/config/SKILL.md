...
- 레코드 `{ model, effort? }`: model은 `^[A-Za-z0-9._-]+$`, effort는 `minimal|low|medium|high|xhigh|max|tiered`이며 `tiered`는 `executor`만 사용할 수 있다. effort 생략은 해당 슬롯의 기본 effort다. 레코드는 닫혀 있다 — 멤버는 `model`(필수, 1회)과 `effort`(선택, 1회)뿐이며 그 외 멤버·중복 멤버·같은 슬롯의 중복 자식은 조회 `⚠ 타입 불일치`, 수정 시 `BLOCKED:UNSUPPORTED_LAYOUT`이다.
...
| 자식 줄 | 들여쓴 `- 항목`(array 블록 시퀀스) 또는 들여쓴 `{슬롯}: { … }`(block 매핑 자식 — 슬롯은 `orchestrator` · `executor` · `readonly` · `advisor`) |
...
| 중립 줄 | 그 외 주석만 있는 줄(들여쓰기 무관 — 두 번째 이후의 동명 플레이스홀더, 활성 키가 있는 키의 `# K:`, 분류표 밖 키의 주석 포함)·빈 줄 |
   ...
   - 블록 범위 = 키 줄부터 **연속된** 자식 줄(두 형태 모두)까지. 그 뒤의 중립 줄은 블록 밖이다. 키 줄과 첫 자식 사이의 중립 줄, 블록 범위 밖에 남는 들여쓴 자식형 줄은 비지원 레이아웃(대상일 때 `BLOCKED:UNSUPPORTED_LAYOUT`)이다.
...
| block | 블록 매핑 = `키:` + 연속 자식 `  {슬롯}: { model: …, effort: … }`(자식은 1줄 flow 매핑, 값은 bare 또는 따옴표) / 빈 flow `{}` / 자식 없는 block: 플레이스홀더 잔여부 빈 값 또는 활성 `{}` | 여러 줄 자식, 중첩, 자식 사이의 중립 줄, 알 수 없는 슬롯 키, flow 매핑이 아닌 자식 값 |
...
| ②′ | 활성 키 + 블록 매핑(block) | 슬롯별로 자식 줄을 재구성한다 — 기존 슬롯 자식은 같은 줄에서 값만 교체(들여쓰기·구분 공백·꼬리 주석·EOL 보존), `default` 슬롯의 자식 줄은 삭제(꼬리 주석이 있으면 Step 3.2 차단), 신규 슬롯은 마지막 자식 뒤에 추가(들여쓰기·EOL = 마지막 기존 자식, 없으면 공백 2·키 줄 EOL), 전 슬롯 default면 자식 삭제 + 키 줄 `{}`. 키 줄 렉심이 `{}`이면 렉심을 제거(`키:` 뒤 구분 공백·꼬리 주석 보존)하고 키 줄 바로 다음에 자식을 삽입한다(EOL = 키 줄) |
| ③ | 키 부재 + 플레이스홀더 줄 | 선두 `# `만 제거해 활성 키 줄로 전환한 뒤 ①(스칼라/flow)을 적용한다. 그 줄의 꼬리 주석·EOL을 보존하고 **뒤따르는 주석 줄(예시·설명)은 손대지 않는다**. block은 잔여부가 비어 있으면 그대로 활성 키 줄로 전환한 뒤 키 줄 **바로 다음**에 자식 줄을 삽입한다(EOL = 키 줄) — 뒤따르는 주석 예시 줄은 그대로 블록 뒤에 남는다. |
