...
#
# canonical: codex-be-harness/skills/e2e-test/assets/e2e-lock.sh
# version:   1.0.0
#
# 사용법:
...
# 여러 셸 호출에 걸쳐 있어 프로세스 수명으로 락을 유지할 수 없다.

set -uo pipefail

VERSION="1.0.0"

DEFAULT_TIMEOUT=540   # 단독 실행 기본 총 대기 상한(초). 스킬은 55초 slice로 재정의한다
...
# --- 락 루트 해석 -------------------------------------------------------------
# ① $HARNESS_E2E_LOCK_DIR  ② $WORK_LOG_ROOT  ③ fallback /tmp/harness-e2e-locks
# 설정 파일을 추측해서 읽지 않는다. 프로젝트별 경로가 필요하면 환경 변수로
# 절대경로를 명시한다.
resolve_lock_root() {
  local vault=""
...
# 토큰은 로컬에만 둔다 — 개별 셸 호출 사이에 변수가 유지되지 않으므로
...
cmd_acquire() {
  ...
  local mine deadline waited age holder remaining sleep_for
  mine="$(load_token)"
