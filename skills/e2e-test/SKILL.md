...
확보한 계정의 실제 역할·소유 자원을 시나리오와 대조한다. 관리자 토큰으로 다른 역할을 대신하지 않는다. 일부 역할 토큰·자원만 부족하면 해당 케이스를 `UNCOVERED:{사유}`로 남기고 가능한 케이스를 진행한다. 필요한 인증 자체가 없는 workflow의 `SKIPPED:NO_AUTH`와 standalone 질문 경로는 아래 계약대로 유지한다. 발급 방법만 기록하고 토큰 값은 Spec·위임·리포트에 복제하지 않는다.
