...
상위 실행은 `{RESULTS_FILE}={RUN_DIR}/verification-results.json`을 사용한다. 독립 E2E 루프는 `{E2E_RUN_DIR}/e2e-results.json`을 사용하고, 상위 RUN_ID가 있으면 승계한다. 단독 루프도 workflow_run.py create로 받은 RUN_ID를 사용한다. 파일은 저장소 밖 실행 디렉터리에 둔다.

```bash
python3 "{PLUGIN_ROOT}/skills/start-workflow/assets/workflow_results.py" init \
  --out "{RESULTS_FILE}" --run-id "{RUN_ID}" --domain be --mode "{build|analyze|verify}" --cwd "{CWD}"
```
...
```bash
python3 "{PLUGIN_ROOT}/skills/start-workflow/assets/workflow_results.py" validate "{RESULTS_FILE}" --run-id "{RUN_ID}"
```
