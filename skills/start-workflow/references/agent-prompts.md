...
작업 계약: {정확한 브랜치·이슈/문서·식별자, 현재 기준 문서·선례 선택 이유, 배정 범위의 완료 증거 — Phase 5부터 Spec의 필요한 섹션만}
승인된 범위·효과: {대상 Spec/Plan·외부 효과와 사용자 승인 근거 원문}
미결 결정: {아직 답이 필요한 항목 또는 없음}
반환 계약: {해당 Phase 출력 형식}
```

`{STATE_FILE}`과 `{IMPL_NOTES}`는 고정 경로로 추측하지 말고 받은 절대 경로만 사용한다. Orchestrator만
상태와 Phase Results를 작성하며 Worker/Readonly/Advisor는 `{STATE_FILE}`과 `{IMPL_NOTES}`를 직접 쓰지 않고
구조화 결과만 반환한다. Worker의 작업 트리 편집 권한은 전달된 파일 소유권 범위에서 유지한다.
...
탐색·수집 질문이 3개 이상이면 3~10개를 한 Readonly 읽기 전용 역할에 묶는다. 한 질문마다 새 역할을 만들지
...
Orchestrator가 Worker의 구조화 결과를 받은 뒤 기록할 때만 사용한다.

```text
설계 결정·편차·트레이드오프·미결 질문을 반환 결과에 명시한다. Orchestrator가 필요 시 {IMPL_NOTES}의
해당 섹션에 append한다. 은 반드시 ## 편차에도 같은 항목으로 기록한다.
```

## 대기 규약

서브에이전트 결과는 단일 `wait_agent`로 기다린다. 역할별 1회 타임아웃은 Readonly 리뷰/스캔 10분, Worker Red 10분,
Worker Green·8.5 수정·E2E 30분, Advisor 15분, Worker 문서/PR 10분이다.
...
Advisor Phase 4.3이 실행 중 두 번 사망하면 타 모델 대체 없이 `agent_died(...)`와 필요한
`agent_retry(...)`를 진단에 남기고 기존 `CODEX-UNAVAILABLE` 결과로 Phase 4.4에 진행한다.

| 두 번 실패한 역할 | 처리 |
|-------------------|------|
| Phase 8.8 격리 Read-back | `SKIPPED:AGENT_DIED`; 오케스트레이터 대체 금지 |
| Readonly 읽기 전용 스캔·리뷰·성찰 | Orchestrator 축소 읽기 검토; `DONE` + `degraded_fallback(...)` |
| Worker 구현·수정·문서·E2E·승인된 push/PR | `BLOCKED:AGENT_DIED`; Orchestrator가 작업 트리/원격 효과를 대체하지 않음 |
...
Phase 4.2 Plan 리뷰어는 Readonly이고 Spec과 Plan을 읽기만 한다. 각 관점마다 다음 형식으로 반환한다.
...
Phase 4.3 fresh-context architect는 resolved concrete advisor effort일 때만 Advisor로 iteration마다 새 `fork_turns:none` context로 만든다.
`tiered`·`N/A`는 spawn하지 않는다. 이전 결론을 답으로 주입하지 않고, 정해진 리뷰 관점과 Spec/Plan의 가장 중요한 결정 질문 1개만 전달해 독립 평가한다.

## Phase 6.2 Green

역할 계약은 Worker [agents/workflow-implementer.md](agents/workflow-implementer.md)를 함께 전달한다.
...
Worker는 커밋·상태 기록을 하지 않는다. `parallel-slices`에서는 각 역할에 정확한 파일 범위를
전달하고 커밋·빌드를 금지한다. Orchestrator가 모든 범위 밖 변경 여부를 검사한 뒤 단일 커밋을 조정한다.
...
Worker는 구조화 결과를 반환하고 Orchestrator가 성공한 수정만 `Fix: 빌드 에러 수정 (Phase 7)`로 커밋을 조정한다.

## Phase 9 API docs

```text
Worker가 {apiDocsPath}의 포맷을 내용으로 판정한다. 이번 Spec에서 추가·변경된 endpoint와 field만 파일에
반영한다. 외부 서비스에는 전송하지 않는다. 문서 diff와 반영 목록을 반환한다.
```

## Phase 8.6 E2E

Orchestrator가 `../../e2e-test-loop/SKILL.md`의 계약과 절대 asset 경로, `{PROFILE_PATH}`(식별용)와 `## Profile Snapshot` 전문(resolved 경로 포함)을 같은 Worker에 전달한다. Worker는 e2e-test-loop 호출에 그 스냅샷을 `mode: workflow` 입력으로 그대로 넘기고, 어느 단계에서도 profile 파일을 다시 읽지 않는다.
호출에는 `mode: workflow`를 명시해 인증 토큰 확보 실패가 사용자 질문 대신 `SKIPPED:NO_AUTH`로 끝나게 한다.
Worker는 중첩 agent spawn이나 직접 commit 없이 E2E와 실패 수정을 수행하고, PID/세션 핸들·정리 결과·
수정 파일·검증 결과를 구조화해 반환한다. Orchestrator만 `{STATE_FILE}` 기록과 commit 조정을 한다.

## Phase 10 PR

Orchestrator가 먼저 Assumption Gate를 통과시킨다. Worker가 일반 모드는 `../../commit-pr/SKILL.md`, hard
...
Readonly 역할에 [agents/workflow-reflection.md](agents/workflow-reflection.md)를 전달한다. 상태 파일과 base
