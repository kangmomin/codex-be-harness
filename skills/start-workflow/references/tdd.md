...
> **테스트 작성 규칙 자체의 canonical은 형제 `../../unit-test/SKILL.md`다.** 이 문서는 워크플로우 통합(baseline·배리어·Test Map·충돌 판정)만 정의한다.
...
| # | 조건 | 상태 코드 |
|---|------|----------|
| 1 | 파싱한 실행 인자에 `--no-tdd` | `SKIPPED:USER_OPT_OUT` |
...
오케스트레이터는 먼저 `../../unit-test/SKILL.md`를 읽고 해당 절차의 Step 1~4 계약을 프롬프트에
포함한다. 스킬 이름 문자열이나 상대 경로만 전달한 채 서브에이전트가 재탐색하게 하지 않는다.
...
> **자율 실행 구간 예외**: Phase 6은 유저 질문이 금지된 구간이다. 따라서 상한 도달 시의 번호 선택지는
> **Phase 12로 이연**한다. `BLOCKED:*`를 기록하되 워크플로우는 멈추지 않는다.
