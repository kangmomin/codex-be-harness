...
> 플레이스홀더(`{STATE_FILE}`·`{PLAN_MAX}`·`{QL_MAX}`·`{SKILL_DIR}`·`{REPORT_DIR}` 등)는 SKILL.md 본문 정의를 따른다.
...
**산정 규칙**
- 각 축 점수 = 요소별 밴드 점수의 **최댓값**(평균 금지). 종합 난이도 = `max(A, B)` (executor 기본은 `high`; 명시적 legacy `tiered`만 난이도 1~8 `high` / 9~10 `max` — `agent-topology.md`; 고정 effort는 난이도와 무관).
...
- 근거 자료: Spec `참조 구현` 열의 경로로 아래를 실행해 출력(존재·최근 변경 커밋 수·동반 테스트·과거 워크플로우 이력)을 B축 `변경 영역 기존 테스트`·`기존 동작 변경 범위`의 근거로 쓴다. 경로가 없거나 스크립트가 exit ≠ 0이면 해당 행은 `UNKNOWN`.
  ```bash
  python3 {SKILL_DIR}/assets/risk_facts.py --paths {참조 구현 경로들} --report-dir {REPORT_DIR} [--test-dir {설정된 testDir} …]
  ```
...
- `light` 강제 플래그는 없다. 점수는 planning-only 구간(Phase 1~4.4)의 대화에서 유저가 근거를 제시하면 재산정할 수 있으나 게이트 조건 자체는 불변.
- 출력: `검증 티어: light|standard — A [a]/B [b], 금지 조건 [해당 없음|{항목}], [TDD off|parallel-slices|--tier standard 로 standard]` — Plan과 함께 Phase 4.4 실행 승인 블록에서 승인.
